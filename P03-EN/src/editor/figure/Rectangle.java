...
public class Rectangle implements Figure {
    ...
    public void move(Point startPoint, Point endPoint) {
        this.corner.translate(endPoint.getX() - startPoint.getX(),
                endPoint.getY() - startPoint.getY());
    }

    @Override
    public boolean isClickInside(Point p) {
        return (this.corner.getX() <= p.getX() && p.getX() <= this.corner.getX() + this.width) && (this.corner.getY() <= p.getY() && p.getY() <= this.corner.getY() + this.height);
    }
